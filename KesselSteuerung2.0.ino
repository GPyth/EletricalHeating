...
HttpClient HTTPclient = HttpClient(client, "api.openweathermap.org", 80);
...
{  
  ...
  if (sI==-1) 
    { 
      int month;
      month=timeClient.getMonth()-1;
      if (h > 0) Temperature=low_t[month];
      if (h > 6) Temperature=(low_t[month]+high_t[month])/2;
      if (h > 12) Temperature=high_t[month];
      if (h > 18) Temperature=(low_t[month]+high_t[month])/2;   
...
void relaisSet(){
  ...
  Hrelais = m <= intervall && kessTemp > 35 ;  //Macht wenig Sinn mit kaltem Kessel zu heizen
